classdef (ConstructOnLoad=true) TLargeLoopAntenna < TAntenna
    % Large Loop Antenna.
    ...
    properties % Specific Properties
        diameter=1;    % Dipole Legth in meters
        Rin=0;            % Circuit resistance (before the antenna)        
    end
    properties (Constant)
        GUIName='Large Loop'; %EVERY ANTENNA SHOULD DEFINE a GUINAME (see TAntenna)        
    end
    
    methods        
        function obj=TLargeLoopAntenna   %CONSTRUCTOR MUST NOT TAKE ARGUMENTS!   
        end
        ...
        function [Er Ef Eth Hr Hf Hth]=fieldAt(obj, r, ~, th, Environment)
            ...
            Er=0;
            Eth=0;
            Ef=a*k*Z*Io*exp(-1i*k*r)*bessel(1,k*a*sin(th))/(2*r);
            Hr=0;
            Hf=0;
            Hth=-a*k*Io*exp(-1i*k*r)*bessel(1,k*a*sin(th))/(2*r);            
        end   
