classdef TPowerdBmWMeasurement < TMeasurement
    
    properties (Constant)
        GUIName='Power (dBmWatt)'; %EVERY MEASUREMENT SHOULD DEFINE a GUINAME (see TAntenna)        
    end
    
    methods 
        function obj=TPowerdBmWMeasurement %MUST NOT TAKE ARGUMENTS  
        ...
        function paint2D(obj, environment)
            ...
            for t=0.0001:1/50:0.999999
                ...
                [f, th, r]=cart2sph(np(1),np(2),np(3));  %matlab coord crap
                th=0.5*pi-th;                            %matlab coord crap
                P(K)=environment.Sender.Antenna.PowerAtPoint(r, f, th, environment);                
