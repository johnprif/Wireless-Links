classdef TSenderDirectivityDBMeasurement < TMeasurement
    
    properties (Constant)
        GUIName='Sender''s Directivity - dB'; %EVERY MEASUREMENT SHOULD DEFINE a GUINAME (see TAntenna)        
    end
    
    methods 
        function obj=TSenderDirectivityDBMeasurement %MUST NOT TAKE ARGUMENTS  
        ...
        function paint1D(obj, environment)
            ...
            P=environment.Sender.Antenna.Directivity(r, f, th, environment);
...
%            P=1000*P;            %m
        ...
        function paint2D(obj, environment)
            ...
            for t=0.0001:1/50:0.999999
                ...
                [f, th, r]=cart2sph(np(1),np(2),np(3));  %matlab coord crap
                th=0.5*pi-th;                            %matlab coord crap
                P(K)=environment.Sender.Antenna.Directivity(r, f, th, environment);                
        ...
        function paint3D(obj, environment)
            ...
            for i=1:I
                for j=1:J
                    ...
                    [f, th, r]=cart2sph(np(1),np(2),np(3));  %matlab coord crap
                    th=0.5*pi-th;                            %matlab coord crap
                    P(i,j)=environment.Sender.Antenna.Directivity(r, f, th, environment);                                   
                end
            end
            P=10*log10(P);       %dB
%            P=10*log10(1000*P);  %dBm
